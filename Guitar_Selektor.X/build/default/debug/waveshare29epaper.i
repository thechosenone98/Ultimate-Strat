...
# 8 "./waveshare29epaper.h" 2

# 1 "C:\\Program Files\\Microchip\\xc8\\v2.32\\pic\\include\\c99\\stdbool.h" 1 3
# 9 "./waveshare29epaper.h" 2
...
void SetFrameMemoryPartial(const uint8_t* image_buffer, uint16_t x, uint16_t y, uint16_t image_width, uint16_t image_height, _Bool invert_color){
    ...
    x &= 0xF8;
    image_width &= 0xF8;
    if (x + image_width >= 128) {
        x_end = 128 - 1;
    } else {
        x_end = x + image_width - 1;
    }
    if (y + image_height >= 296) {
        y_end = 296 - 1;
    } else {
...
void SetFrameMemoryFull(const uint8_t* image_buffer, _Bool invert_color) {
    SetMemoryArea(0, 0, 128 - 1, 296 - 1);
    SetMemoryPointer(0, 0);
    SendCommand(0x24);

    uint8_t data;
    uint16_t i;
    for (i = 0; i < 128 / 8 * 296; i++) {
        data = image_buffer[i];
        SendData(invert_color ? ~data : data);
    }
}
